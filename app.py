...
target_domain = st.sidebar.selectbox("Google Domein", ["google.nl", "google.be", "google.com"])
...
# Woorden voor partner-links
PARTNER_TERMS = ["partner", "adverteren", "samenwerken", "samenwerking", "gastblog", "advertise"]
...
def extract_domain(val):
    ...
    if '://' in val: val = urlparse(val).netloc
    return val.replace('www.', '')

def ai_analyze(text, url, ai_client):
    try:
        prompt = f"""
        Je bent een SEO-expert. Analyseer deze 'Partner' pagina voor linkbuilding: {url}
        Tekst: {text[:2000]}
        Output format:
        SCORE: [X/10] | TYPE: [Gastblog/Betaald/Onbekend] | ANALYSE: [Korte bulletpoints]
        """
...
def process_site(home_url, ai_client):
    try:
        # 1. Bezoek homepage
        ...
        for link in soup.find_all('a', href=True):
            if any(t in link.get_text().lower() for t in PARTNER_TERMS):
...
if st.button("🚀 Start Analyse", type="primary"):
    if not api_token or not oa_token or not uploaded_file or not keywords_area:
        ...
        with st.status("Bezig met scrapen en analyseren...", expanded=True) as status:
            ...
            st.write("🔎 Domeinen filteren en partnerpagina's scannen...")
            for item in apify.dataset(run["defaultDatasetId"]).iterate_items():
                # Correcte keyword extractie
                ...
                for result in item.get('organicResults', []):
                    ...
                    if dom not in existing:
                        ...
                        analysis = process_site(url, openai_c)
                        if analysis:
