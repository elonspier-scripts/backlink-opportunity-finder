...
## Backlink engine add-on

- **Backlink-focused scoring:** add a weighted score that combines topical relevance, domain quality, outbound link quality, and editorial fit.
- **Opportunity types:** classify pages as `guest post`, `sponsored`, `resource page`, `directory`, or `partnership`.
- **Anchor and pitch guidance:** generate anchor text suggestions and a short outreach angle per lead.
- **Domain overlap checks:** detect if a domain is already linking to you and downgrade duplicate opportunities.
- **Priority tiers:** assign `high`, `medium`, or `low` backlink potential for faster qualification.
